...
from config.config import SESSIONS_UPLOAD_DIR, ConfigManager
...
async def handle_pagination(callback_query: CallbackQuery, callback_data: MyCallback):
    ...
    if action == "delete":
        await delete_directory(f'{SESSIONS_UPLOAD_DIR}/{account_id}')
        await AccountService.clear_cache()
        await callback_query.message.edit_reply_markup(reply_markup=await get_accounts_keyboard(page))
    elif action == "first":
