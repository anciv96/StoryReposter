...
async def delay_handler(message: Message, state: FSMContext) -> None:
    await message.answer('Введите задержу между постингом сторис в секундах')
...
async def get_delay_handler(message: Message, state: FSMContext) -> None:
    ...
    try:
        ...
        await message.answer('Задержка должна быть целым числом.')
        return

    await ConfigManager.set_setting(key='posting_delay', value=delay)
    await message.answer(f'Вы успешно установили время {delay}', reply_markup=ReplyKeyboardRemove())
