...
async def get_file_handler(message: Message, state: FSMContext) -> None:
    ...
    file_path = os.path.join(SESSIONS_UPLOAD_DIR, message.document.file_name)
    if not (message.document.mime_type in ['application/zip',
                                           'application/x-rar-compressed',
                                           'application/octet-stream'
                                           ]):
        await message.answer('Отмена импорта...')
        await message.reply("Неизвестный формат архива. Поддерживаются только ZIP и RAR.")
        await state.clear()
        return
...
async def _extract_archive_file(message: Message, file_path: str):
    ...
    elif message.document.mime_type in ['application/x-rar-compressed', 'application/octet-stream']:
...
async def get_proxy(message: Message, state: FSMContext) -> None:
    ...
    if message.text != '-':
        ...
        await message.answer('Прокси сохранены.')
    else:
        await message.answer('Архив аккаунтов сохранен.')
